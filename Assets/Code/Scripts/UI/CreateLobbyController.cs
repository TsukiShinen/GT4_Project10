...
public class CreateLobbyController : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		Root.Q<Button>("Quit").clicked += () => { Root.style.display = DisplayStyle.None; };
		...
		{
			LobbyManager.Instance.CreateLobby(m_Name.value,
				m_GameModes.GameModeConfigs.Find(g => g.ModeName == m_GameMode.value), m_IsPrivate.value);
		};
	}
